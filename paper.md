...
There is only a handful of full-fledged accelerated open-source flow solver packages available in the literature. Some selected examples for compressible flows include (i) popular multi-purpose open-source packages [@Nektar,@SU2] and OpenSBLI [@OpenSBLI], which is a Python-based framework for the automated derivation of finite-difference solvers for hybrid (CPU-GPU) architectures, (ii) HTR [@HTR] based on the task-based Legion programming paradigm [@Legion] and targeted to simulate hypersonic reacting flows, and (iii) STREAmS [@STREAmS] resulting from the extension of a CPU-based compressible flow solver to run on multi-GPU clusters and focused on supersonic ideal-gas flows. In this regard, RHEA has been designed and developed with the aim to cover this void for the academic community, specifically focusing on (i) ease of use, (ii) generality of flow problems that can be studied, (iii) rapid portability to different architectures, and (iv) outstanding computational performance.
